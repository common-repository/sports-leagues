( function( $ ) {
		...
		$( '.anwp_sl_cmb_select2' ).each( function() {
			$( this ).select2( {
				allowClear: true
			} );
		} );
		...
		$( '.anwp_sl_cmb_select2_multi' ).each( function() {
		...
		$( '.cmb-repeatable-group' ).on( 'cmb2_add_group_row_start', function( event, instance ) {
			...
			$oldRow.find( '.anwp_sl_cmb_select2' ).each( function() {
		...
		$( '.cmb-repeatable-group' ).on( 'cmb2_add_row', function( event, newRow ) {
			$( newRow ).find( '.anwp_sl_cmb_select2' ).each( function() {
			...
			$( newRow ).find( '.anwp_sl_cmb_select2_multi' ).each( function() {
				$( 'option:selected', this ).removeAttr( 'selected' );
				$( this ).select2_sortable();
			} );

			// Reinitialise the field we previously destroyed
			$( newRow ).prev().find( '.anwp_sl_cmb_select2' ).each( function() {
				$( this ).select2( {
					allowClear: true
				} );
			} );
			...
			// Reinitialise the field we previously destroyed
			$( newRow ).prev().find( '.anwp_sl_cmb_select2_multi' ).each( function() {
				$( this ).select2_sortable();
			} );
		} );
		...
		$( '.cmb-repeatable-group' ).on( 'cmb2_shift_rows_start', function( event, instance ) {
			...
			groupWrap.find( '.anwp_sl_cmb_select2' ).each( function() {
				$( this ).select2( 'destroy' );
			} );
		...
		$( '.cmb-repeatable-group' ).on( 'cmb2_shift_rows_complete', function( event, instance ) {
			...
			groupWrap.find( '.anwp_sl_cmb_select2_multi' ).each( function() {
				$( this ).select2_sortable();
			} );
		} );
