<?php
/**
 * Customize page for Sports Leagues
 *
 * @link       https://anwp.pro
 * @since      0.14.0
 *
...
$is_mode_active = 'no' !== get_option( 'anwp_sl_customizer_mode' );
?>
<div class="anwp-b-wrap" style="max-width: 1400px; margin-top: 30px;">
	<div class="postbox">
		<div class="inside">
			<h1 class="text-left text-uppercase m-3 anwp-text-gray-600"><?php echo esc_html__( 'Customize Options', 'sports-leagues' ); ?></h1>
			<div class="d-flex mt-4">
				<div class="anwp-flex-1 py-3 px-4 m-3 anwp-border anwp-border-gray-300" style="border-width: 2px; border-radius: 2px;">
					...
					<div class="my-4 anwp-text-base anwp-text-teal-800 p-2 anwp-bg-teal-100 anwp-border anwp-border-teal-300">
						...
						<div>2) activate Sports Leagues Mode (activated by default)</div>
					...
					<div class="d-flex">
						...
						<div class="d-flex flex-column">
							<h3 class="mt-0 mb-1 anwp-text-blue-700">SiteOrigin CSS + Special Sports Leagues Mode *</h3>
					...
					<p class="anwp-text-xs my-3">* Special mode allow to select only recommended classes (selectors) from Sports Leagues template files. See tutorial below for detailed info. If you want to style other site parts, disable Sports Leagues Mode.</p>
					...
					if ( ! defined( 'SOCSS_VERSION' ) && current_user_can( 'install_plugins' ) ) :
						...
						if ( $plugin_installed && current_user_can( 'activate_plugins' ) ) :
							?>
							<a href="<?php echo esc_url( wp_nonce_url( 'plugins.php?action=activate&plugin=' . rawurlencode( 'so-css/so-css.php' ), 'activate-plugin_so-css/so-css.php' ) ); ?>" class="button button-primary"><?php echo esc_html__( 'Activate SiteOrigin CSS', 'sports-leagues' ); ?></a>
						<?php elseif ( current_user_can( 'install_plugins' ) ) : ?>
							<a href="<?php echo esc_url( wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=so-css' ), 'install-plugin_so-css' ) ); ?>" class="button button-primary"><?php echo esc_html__( 'Install SiteOrigin CSS', 'sports-leagues' ); ?></a>
						<?php endif; ?>
					<?php endif; ?>
					<hr class="mb-2 mt-4">
					<div class="d-flex flex-wrap align-items-center anwp-bg-gray-100 p-2">
						<span class="anwp-text-sm">Sports Leagues Mode: </span>
					...
					<a target="_blank" href="https://anwppro.userecho.com/en/knowledge-bases/2/articles/1959-customize-football-leagues-with-siteorigin-css-special-mode-wordpress-plugin">
						- Customize Sports Leagues with SiteOrigin CSS
					</a>
				</div>
				<div class="anwp-flex-1 py-3 px-4 m-3 anwp-border anwp-border-gray-300" style="border-width: 2px; border-radius: 2px;">
					...
					<a target="_blank" href="<?php echo esc_url( admin_url( 'customize.php?autofocus[panel]=anwp_sl_panel' ) ); ?>"
...
<script>
	(function( $ ) {
		...
		$( function() {
			...
			let $badgeMode = $( '#anwp-customize-mode-status' );
			let $btnChange    = $( '#anwp-customize-change-btn' );
			...
			$btnChange.on( 'click', function( e ) {
				...
				jQuery.ajax( {
					...
					url: anwp.rest_root + 'sports-leagues/v1/customize/toggle-mode/'
				} ).done( function() {
