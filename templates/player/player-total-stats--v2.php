...
<div class="anwp-section player-total-stats table-responsive" data-version="v2">
	...
	<?php if ( ! empty( $player_stats ) && is_array( $player_stats ) ) : ?>
		<table class="table table-bordered anwp-stats-table table-sm anwp-sl-player-total-stats w-100 my-0 nowrap">
			...
			foreach ( $player_stats as $team_id => $team_data ) :
				...
				$player_stat_group = sports_leagues()->player_stats->get_player_stat_group( $player_position, $stat_groups );
				...
				if ( $roster_position !== $player_position ) {
					$maybe_player_stat_group = sports_leagues()->player_stats->get_player_stat_group( $roster_position, $stat_groups );
					$player_stat_group       = ( $maybe_player_stat_group && $maybe_player_stat_group !== $player_stat_group ) ? $maybe_player_stat_group : $player_stat_group;
				}

				if ( empty( $player_stat_group ) ) {
					continue;
				}
				...
				if ( ! $thead_loaded ) :
					...
					<tr class="small">
						...
						foreach ( $stats_columns_season as $column ) :
							if ( 'hidden' === $column->visibility || empty( $column->groups || ! in_array( $player_stat_group, $column->groups, true ) ) ) {
								continue;
							}
							?>
