...
if ( ! class_exists( 'AnWP_SL_CMB2_Field_Select2' ) ) :

	class AnWP_SL_CMB2_Field_Select2 {
		...
		public function __construct() {
			add_filter( 'cmb2_render_anwp_sl_select', [ $this, 'render_sl_select' ], 10, 5 );
			add_filter( 'cmb2_render_anwp_sl_multiselect', [ $this, 'render_sl_multiselect' ], 10, 5 );
			add_filter( 'cmb2_sanitize_anwp_sl_multiselect', [ $this, 'multiselect_sanitize' ], 10, 4 );
			add_filter( 'cmb2_types_esc_anwp_sl_multiselect', [ $this, 'multiselect_escaped_value' ], 10, 3 );
		 ...
		 * @version 1.0.0
		 */
		public function render_sl_select( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {

			$attr = [
				'class'            => 'anwp_sl_cmb_select2',
		 ...
		 */
		public function render_sl_multiselect( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {

			$options = [
				'multiple'         => 'multiple',
				'style'            => 'width: 99%',
				'class'            => 'anwp_sl_cmb_select2_multi',
				'name'             => $field_type_object->_name() . '[]',
				'id'               => $field_type_object->_id(),
				'desc'             => $field_type_object->_desc( true ),
				'options'          => $this->get_sl_multiselect_options( $field_escaped_value, $field_type_object ),
				'data-placeholder' => $field->args( 'attributes', 'placeholder' ) ? $field->args( 'attributes', 'placeholder' ) : $field->args( 'description' ),
			];

			$a = $field_type_object->parse_args( 'anwp_sl_multiselect', $options );
		...
		/**
		 * Return list of options for anwp_sl_multiselect
		 *
		 ...
		 */
		public function get_sl_multiselect_options( $field_escaped_value, $field_type_object ) {
		...
		public function multiselect_table_row_class( $check ) {
			$check[] = 'anwp_sl_cmb_select2_multi';
...
$anwp_sl_cmb2_field_select2 = new AnWP_SL_CMB2_Field_Select2();
