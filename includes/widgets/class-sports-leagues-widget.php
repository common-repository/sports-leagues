...
abstract class Sports_Leagues_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		foreach ( $widget_fields as $field ) {
			...
			if ( ! empty( $field['before_field'] ) ) {
				echo $field['before_field']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			switch ( $field['type'] ) {

				case 'docs':
					$field['url'] = isset( $field['url'] ) ? $field['url'] : '';
					?>
					<div class="anwp-widget-docs">
						<span class="dashicons dashicons-editor-help"></span>&nbsp;<a target="_blank" href="<?php echo esc_url( $field['url'] ); ?>"><?php echo esc_html__( 'Widget Documentation', 'sports-leagues' ); ?></a>
					</div>
					<?php
					break;
				...
				case 'text':
					...
					<p>
						...
						<?php if ( ! empty( $field['description'] ) ) : ?>
							<span class="anwp-widget-desc"><?php echo esc_html( $field['description'] ); ?></span>
						<?php endif; ?>
					</p>
					<?php
					break;
				...
				case 'select_posts':
					...
					<p>
						...
						<select
							id="<?php echo esc_attr( $this->get_field_id( $field['id'] ) ); ?>"
							name="<?php echo esc_attr( $this->get_field_name( $field['id'] ) ); ?>"
							class="widefat anwp-sl-widget-select <?php echo esc_attr( $field['classes'] ); ?>">
				...
				case 'player_id':
					...
					<div class="anwp-mt-2">
						...
						<div class="anwp-d-flex">
							...
							<button type="button" class="button anwp-sl-selector anwp-sl-selector--visible anwp-ml-2" style="display: none;" data-context="player"
				...
				case 'team_id':
					...
					<div class="anwp-mt-2">
						...
						<div class="anwp-d-flex">
							...
							<button type="button" class="button anwp-sl-selector anwp-sl-selector--visible anwp-ml-2" style="display: none;" data-context="team"
								data-single="<?php echo esc_attr( 'no' === $field['single'] ? 'no' : 'yes' ); ?>">
								<span class="dashicons dashicons-search"></span>
							</button>
						</div>
				...
				case 'game_id':
					...
					<div class="anwp-mt-2">
						...
						<div class="anwp-d-flex">
							...
							<button type="button" class="button anwp-sl-selector anwp-sl-selector--visible anwp-ml-2" style="display: none;" data-context="game"
								data-single="<?php echo esc_attr( 'no' === $field['single'] ? 'no' : 'yes' ); ?>">
								<span class="dashicons dashicons-search"></span>
							</button>
						</div>
			...
			if ( ! empty( $field['after_field'] ) ) {
				echo $field['after_field']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
	}
	...
	public function widget( $args, $instance ) {
		...
		// Display the widget.
		echo $this->get_widget( $atts ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	public function get_widget( $atts ) {
		...
		// Prepare widget name
		$widget_name = str_replace( 'sl-widget-', '', $this->get_widget_slug() );

		return sports_leagues()->template->widget_loader( $widget_name, $atts );
	}
